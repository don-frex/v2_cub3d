...
/*   Updated: 2023/12/19 20:46:08 by asaber           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double e_p, double id)
{
	while (s_p >= 0)
	{
		mlx_put_pixel(g_info->image, id, s_p, ft_pixel(197, 255, 248, 255));
		s_p--;
	}
	while (e_p <= g_info->hight)
	{
		mlx_put_pixel(g_info->image, id, e_p, ft_pixel(245, 232, 199, 255));
		e_p++;
	}
}
